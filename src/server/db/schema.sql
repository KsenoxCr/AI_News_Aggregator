...
CREATE TABLE user (
    ...
    selected_agent  VARCHAR(36) REFERENCES user(id) ON DELETE CASCADE,
...
CREATE TABLE agent ( -- TODO: schema enforced single enabled agent per used
...
CREATE TABLE cached_article (
    id            VARCHAR(36)  PRIMARY KEY,
    source_id     VARCHAR(36)  NOT NULL REFERENCES source(id) ON DELETE CASCADE,
...
CREATE TABLE news_digest ( -- TODO: rename to digest_aggregate
    id            VARCHAR(36)  PRIMARY KEY,
    user_id       VARCHAR(36)  NOT NULL REFERENCES user(id) ON DELETE CASCADE,
    updated_at    TIMESTAMP    NOT NULL,
    created_at    TIMESTAMP    NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE digest_revision (
    id            VARCHAR(36)  PRIMARY KEY,
    digest_id     VARCHAR(36)  NOT NULL,
    article_id    VARCHAR(36)  NOT NULL REFERENCES cached_article(id) ON DELETE CASCADE,
    revision      INT UNSIGNED NOT NULL,
    agent_id      VARCHAR(36)  NOT NULL REFERENCES agent(id) ON DELETE CASCADE,
    title         TEXT         NOT NULL,
...
CREATE TABLE digest_source (
    id          VARCHAR(36)   PRIMARY KEY,
    digest_id   VARCHAR(36)   NOT NULL REFERENCES news_digest(id) ON DELETE CASCADE,
    url         VARCHAR(2048) NOT NULL,
    url_hash    VARCHAR(64)   NOT NULL,  -- For indexing compliance with older InnoDB versions
    title       VARCHAR(500),
    source_id   VARCHAR(36) REFERENCES source(id) ON DELETE CASCADE,
...
CREATE TABLE user_category (
  user_id   VARCHAR(36) NOT NULL REFERENCES user(id) ON DELETE CASCADE,
  category  VARCHAR(100) NOT NULL REFERENCES category(slug) ON DELETE CASCADE,
